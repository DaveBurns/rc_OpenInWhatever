...
--
--  ###1 It seems this function is not being called anywhere - certainly xmp-crop could call it, but isn't (I'm guessing I forgot to retrofit or decided not to or something).
--
...
--  @usage ###1 It could be that source and/or dest settings need to be pre-assured for dev transfer - not sure atm.
    ...
    local s, m = app:call( Call:new{ name="Transfer Metadata", main=function( call )
        ...
        if xfrMeta and xfrDev then
            ...
            exifToolSession:addArg( "-all=" ) -- strip all tags from dest file (note: this makes sense if target is xmp sidecar or jpg being sync'd from raw partner, but if one jpg being sync'd from another: only if the other has it all..
                -- ###1 document this too. Note: this does not strip icc-profile, but will strip icc-related support tags.
